#https://github.com/irssi/scripts.irssi.org/blob/master/scripts/il.pl
#
...
sub beancounter {
	...
	#
	$length = Irssi::parse_special ( "\$[-!$width]\@L" );
  # $length = Irssi::parse_special ( "$padChar\@L$padChar" );
    ...
    $sbItem->default_handler ( $get_size_only, "{sb $padChar $length $padChar}", undef, 1 );
}
